...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const init = async () => {
      const { data } = await api_modules_home.cardArticleApi({ category: articleStore.card, pagenum: 1 });
    ...
    return (_ctx, _cache) => {
      return {
        a: common_vendor.f(common_vendor.unref(artilceList), (item, k0, i0) => {
          return {
            a: "http://172.19.10.136:3000" + item.cover[0].imgUrl,
            b: common_vendor.t(item.title),
            c: "http://172.19.10.136:3000" + item.avatar,
